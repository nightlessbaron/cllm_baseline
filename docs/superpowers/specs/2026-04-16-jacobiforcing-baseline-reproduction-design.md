...
**Provenance note worth capturing in the paper's methods section:** the Jacobi trajectories inside `OpenCodeInstruct_training_data_n32w16` were generated by an intermediate `JacobiForcing_Coder_7B_v0` checkpoint (source model declared on the HF dataset card), not by the base `Qwen2.5-Coder-7B-Instruct` and not by the published `_v1` checkpoint. The training data is therefore bootstrapped from an earlier iteration. If a reviewer asks "how were the training trajectories obtained," the honest answer is "we reused the authors' v0-generated trajectories rather than re-rolling from the base model."
...
2. **The five method stages** — each subsection gives *what it does*, *key hyperparameters*, *where it lives in code*:
   ...
   3. **Training-sequence prep with progressive noise window** — `generate_trajectory/data/2_prepare_efficient_cllm_training_data_progressive_noise_window.py` (explain `n_token_seq_length`, `window_size`, `min_noisy_ratio`, `max_noisy_ratio`, `strategy=progressive`). *Under Option A this stage is also not run — the HF dataset ships the output of this stage already (columns: `prompt_ids`, `complete_training_sequence_ids`, `traj_position_indices`, etc.).*
...
- **v0-sourced training trajectories.** The HF training shards were produced by an intermediate `_v0` model. This is a property of the upstream recipe (not a deviation), but must be disclosed — the reproduction is faithful to upstream's recipe, not to a hypothetical "train from scratch using only base Qwen" recipe.
